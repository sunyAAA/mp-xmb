<template>
    <div class="daely-page">
        ...
        <div class="item-box" v-for='(item,index) in renderList' :key="index">
            <span class="delete" v-show='isEdit' @click='removeDaely(item.logId)'>
                删除
            </span>
            <div class="dealy-box">
                ...
                <div class="img-box" v-if="item.showImg">
                    <img v-if='item.imgList[0]' :src="item.imgList[0]" alt="">
                    <img v-if='item.imgList[1]' :src="item.imgList[1]" alt="">
                </div>
            </div>
        </div>
    </div>
...
import {getDaelyByTid, getTargetDetail,removeLogById} from '../../api'
...
export default {
  ...
  computed:{
      ...
      renderList(){
          return this.daelyList.filter(item=>{
  ...
  methods:{
    getDaelyList(){
        ...
        getTargetDetail(this.tid).then(res=>{
            ...
            if(d.code == 1){
                ...
                if(this.uid == d.data.userId){this.isSelf=true};
            }else{
                msg('数据请求失败~！')
            }
        })
        getDaelyByTid(this.tid,this.pageSize,this.pageNum).then(res=>{
            var d = res.data
            if(d.code == 1){
                this.daelyList = d.data
            }else{
                msg('数据请求失败~！')
            }
        })
    ...
    goEdit(){
         wx.navigateTo({ url: '../../pages/editDaely/main?tid='+this.tid });
    }
  }
};
</script>
